...
def form():
    if request.method == 'POST':
        ...
        try:
            ...
            insert_query = "INSERT INTO admin-signup (name, contact, email, password) VALUES (%s, %s, %s, %s)"
...
def userform():
    if request.method == 'POST':
        ...
        try:
            ...
            insert_query = "INSERT INTO user-signup (name, contact, email, password) VALUES (%s, %s, %s, %s)"
            c.execute(insert_query, data)
            conn.commit()
        except pymysql.Error as e:
            print(f"Database error: {e}")
...
def form1():
    if request.method == 'POST':
        ...
        try:
            ...
            select_query = "SELECT * FROM admin-signup WHERE email = %s"
...
def userform1():
    if request.method == 'POST':
        ...
        try:
            ...
            # Query to retrieve user data based on email
            select_query = "SELECT * FROM user-signup WHERE email = %s"
            c.execute(select_query, (email,))
            user = c.fetchone()
